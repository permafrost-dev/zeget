	...
	Describe("Find", func() {
		Context("with a valid tag", func() {
			It("should return assets", func() {
				...
				findResult := assetFinder.Find(client)
		...
		Context("find a match", func() {
			It("should return assets", func() {
				...
				findResult := assetFinder.FindMatch(client)

				Expect(findResult.Error).ToNot(HaveOccurred())
				Expect(findResult.Assets).To(HaveLen(1))
				Expect(findResult.Assets[0].Name).To(Equal("asset1"))
				Expect(findResult.Assets[0].DownloadURL).To(Equal("http://example.com/asset1"))
			})
		})
